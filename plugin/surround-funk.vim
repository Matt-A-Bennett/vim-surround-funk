...
"{{{- parts2string ----------------------------------------------------------
function! Parts2string(parts, word_size)
    let [start_pos, open_pos, trail_pos, close_pos] = Get_func_markers(a:word_size)
    if open_pos[0] == trail_pos[0] && trail_pos[0] == close_pos[0]
        let str = getline('.')
...
function! Operate_on_surrounding_func(word_size, operation)
    ...
    let [result, rm1, rm2] = Parts2string(parts, a:word_size)
...
"{{{---------------------------------------------------------------------------
function! Repeatable_delete(word_size, operation, mapname)
    call Operate_on_surrounding_func(a:word_size, a:operation)
    silent! call repeat#set("\<Plug>".a:mapname, v:count)
endfunction

function! Repeatable_paste(word_size, func_or_word, mapname)
    call Paste_func_around(a:word_size, a:func_or_word)
    silent! call repeat#set("\<Plug>".a:mapname, v:count)
endfunction
